...
const WellnessDashboard = ({ onLogout }) => {
  ...
  const extractTimeFromISO = (isoString) => {
    ...
    try {
      ...
      if (timeMatch) {
        return timeMatch[1]; // Returns HH:MM format
      }
      
      // Fallback to date parsing but force UTC to avoid timezone issues
  ...
  const getEntryDate = (entry, entryType) => {
    switch (entryType) {
      case 'hydration':
        return entry.time_logged || entry.date;
      case 'sleep':
        return entry.bedtime || entry.date;
      default:
        return entry.date;
    }
  };
